...
Set `GEMINI_MODERATION_MODEL` and `GEMINI_SUMMARY_MODEL` as non-secret Wrangler variables. The free-MVP default for both tasks is `gemini-3.5-flash-lite`. The variables remain separate so each task can be migrated independently later. For local AI calls, copy `backend/.dev.vars.example` to `backend/.dev.vars`. Never commit `.dev.vars`, `.env.local`, API keys or credentials.
