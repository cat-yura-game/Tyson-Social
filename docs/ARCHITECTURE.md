...
The recommender is hybrid. A deterministic scoring module combines freshness, normalized popularity, author affinity and explicit events, then the provider-agnostic `RecommendationProvider` may rerank the bounded candidate set. Production uses Gemini while a deterministic provider is available for local development and as a fallback. Gemini receives only public post excerpts and anonymous interaction types—never email, username, message contents or other sensitive attributes. Results are cached in D1 for 15 minutes to control latency and free-tier usage. A hidden dislike is a strong negative signal and is never included in public or author-facing projections. A bounded exploration term preserves discovery. Events are limited to `impression`, `open`, `like`, `dislike` and `comment`.

## AI and abuse prevention

Public posts and comments pass through rule-based checks and a `ModerationProvider` interface whose production implementation is Gemini. The Cloudflare Worker calls Gemini over HTTPS using `GEMINI_API_KEY` from a Cloudflare secret; the browser has no provider access. Moderation, summary and recommendation models are selected independently through `GEMINI_MODERATION_MODEL`, `GEMINI_SUMMARY_MODEL` and `GEMINI_RECOMMENDATION_MODEL` and must pass a server-side allowlist. Structured JSON output is validated again with Zod before it can influence application state. Decisions are `allow`, `review` or `block`, with risk, categories, reason, provider and model version persisted. A Gemini safety-filter response becomes `review`, and one model decision never creates an irreversible account ban.
