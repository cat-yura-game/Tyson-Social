...
struct AITabView: View {
    ...
    var body: some View { NavigationStack { VStack(spacing: 0) {
        Picker("Модель", selection: $model) { Text("Flash Lite").tag("lite"); Text("Flash").tag("flash"); Text("Smart").tag("smart") }.pickerStyle(.segmented).padding()
        ScrollView { LazyVStack(spacing: 12) { if messages.isEmpty { ContentUnavailableView("Tyson AI", systemImage: "sparkles", description: Text("Задайте вопрос или прикрепите изображение либо документ.")) }; ForEach(messages) { message in HStack { if message.role == "user" { Spacer(minLength: 45) }; VStack(alignment: .leading, spacing: 6) { Text(message.role == "assistant" ? "Tyson AI" : "Вы").font(.caption.bold()).foregroundStyle(.secondary); Text(message.content); if let name = message.attachmentName { Label(name, systemImage: "doc.fill").font(.caption) } }.padding(13).background(message.role == "user" ? TysonColor.accent.opacity(0.18) : Color(uiColor: .secondarySystemBackground), in: .rect(cornerRadius: 18)); if message.role != "user" { Spacer(minLength: 45) } } } }.padding() }
        if !attachmentName.isEmpty { HStack { Label(attachmentName, systemImage: attachmentIsImage ? "photo" : "doc"); Spacer(); Button { clearAttachment() } label: { Image(systemName: "xmark.circle.fill") } }.padding(10).background(.thinMaterial) }
        TysonGlass { HStack { Menu { PhotosPicker(selection: $photo, matching: .images) { Label("Изображение", systemImage: "photo") }; Button { showFiles = true } label: { Label("Документ", systemImage: "doc") } } label: { Image(systemName: "paperclip") }; TextField("Сообщение", text: $input, axis: .vertical).lineLimit(1...4); Button { Task { await send() } } label: { Image(systemName: "arrow.up.circle.fill").font(.title) }.disabled((input.isEmpty && attachmentData == nil) || busy) }.padding(10) }.padding(8)
    }.navigationTitle("Tyson AI").navigationBarTitleDisplayMode(.inline).toolbar { ToolbarItem(placement: .topBarTrailing) { Button { Task { await newChat() } } label: { Image(systemName: "square.and.pencil") } } }.task { await load() }.onChange(of: photo) { _, item in Task { if let data = try? await item?.loadTransferable(type: Data.self) { attachmentData = data; attachmentName = "Изображение"; attachmentMime = "image/jpeg"; attachmentIsImage = true } } }.fileImporter(isPresented: $showFiles, allowedContentTypes: [.pdf, .plainText, .json, .commaSeparatedText, .data]) { result in if let url = try? result.get() { let access = url.startAccessingSecurityScopedResource(); defer { if access { url.stopAccessingSecurityScopedResource() } }; attachmentData = try? Data(contentsOf: url); attachmentName = url.lastPathComponent; attachmentMime = UTType(filenameExtension: url.pathExtension)?.preferredMIMEType ?? "application/octet-stream"; attachmentIsImage = false } } } }
    private func load() async { conversations = (try? await TysonAPI.shared.aiConversations()) ?? []; if let first = conversations.first { active = first; messages = (try? await TysonAPI.shared.aiMessages(conversationId: first.id)) ?? [] } }
