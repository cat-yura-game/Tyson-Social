...
struct AITabView: View {
    ...
    var body: some View { NavigationStack { VStack(spacing: 0) {
            Picker("Модель", selection: $model) { Text("Flash Lite").tag("lite"); Text("Flash").tag("flash"); Text("Smart").tag("smart") }
                .pickerStyle(.segmented).padding(6).tysonGlassSurface(Capsule()).padding(.horizontal, 12).padding(.vertical, 8)
            ...
            if !attachmentName.isEmpty { HStack { Label(attachmentName, systemImage: attachmentIsImage ? "photo" : "doc"); Spacer(); Button { clearAttachment() } label: { Image(systemName: "xmark.circle.fill") } }.padding(10).tysonGlassSurface(Capsule()).padding(.horizontal, 8) }
            HStack { Menu { PhotosPicker(selection: $photo, matching: .images) { Label("Изображение", systemImage: "photo") }; Button { showFiles = true } label: { Label("Документ", systemImage: "doc") } } label: { Image(systemName: "paperclip") }; TextField("Сообщение", text: $input, axis: .vertical).lineLimit(1...4); Button { Task { await send() } } label: { Image(systemName: "arrow.up.circle.fill").font(.title) }.disabled((input.isEmpty && attachmentData == nil) || busy) }.padding(10).tysonGlassSurface(Capsule()).padding(8)
        }
...
private struct AIChatsSheet: View {
    ...
    let conversations: [AIConversation]; let activeID: String?; let open: (AIConversation) -> Void
    var body: some View { NavigationStack { List(conversations) { chat in Button { open(chat); dismiss() } label: { HStack(spacing: 12) { Image(systemName: "sparkles").foregroundStyle(TysonColor.accent); VStack(alignment: .leading, spacing: 3) { Text(chat.title).foregroundStyle(.primary).lineLimit(1); Text(date(chat.updatedAt)).font(.caption).foregroundStyle(.secondary) }; Spacer(); if chat.id == activeID { Image(systemName: "checkmark.circle.fill").foregroundStyle(TysonColor.accent) } } } }.navigationTitle("Чаты Tyson AI").toolbar { ToolbarItem(placement: .topBarTrailing) { Button("Готово") { dismiss() } } } } }
