...
struct AITabView: View {
    ...
    }.navigationTitle(active?.title ?? "Tyson AI").navigationBarTitleDisplayMode(.inline).sheet(isPresented: $showChats) { AIChatsSheet(conversations: conversations, activeID: active?.id, create: { Task { await newChat(); showChats = false } }, delete: { conversation in Task { await delete(conversation) } }) { conversation in Task { await open(conversation) }; showChats = false } }.task { await load() }.onChange(of: photo) { _, item in Task { if let data = try? await item?.loadTransferable(type: Data.self) { attachmentData = data; attachmentName = "Изображение"; attachmentMime = "image/jpeg"; attachmentIsImage = true } } }.fileImporter(isPresented: $showFiles, allowedContentTypes: [.pdf, .plainText, .json, .commaSeparatedText, .data]) { result in if let url = try? result.get() { let access = url.startAccessingSecurityScopedResource(); defer { if access { url.stopAccessingSecurityScopedResource() } }; attachmentData = try? Data(contentsOf: url); attachmentName = url.lastPathComponent; attachmentMime = UTType(filenameExtension: url.pathExtension)?.preferredMIMEType ?? "application/octet-stream"; attachmentIsImage = false } } }
    private func load() async { conversations = (try? await TysonAPI.shared.aiConversations()) ?? [] }
    ...
    private func delete(_ conversation: AIConversation) async { guard (try? await TysonAPI.shared.deleteAIConversation(id: conversation.id)) != nil else { return }; conversations.removeAll { $0.id == conversation.id }; if active?.id == conversation.id { active = nil; messages = [] } }
