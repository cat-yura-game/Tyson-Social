import { Bookmark, ChevronLeft, ImagePlus, LockKeyhole, Mic, Plus, Send, Smile, Square } from 'lucide-react';
    ...
    <div className="chat-panel">{active ? <>
      ...
      <div className="composer-area">
        ...
        <form className="message-composer" onSubmit={(event) => void sendText(event)}>
          ...
          <button className="image-message-trigger" type="button" disabled={uploading || sending || recording} aria-label="Отправить изображение" onClick={() => imageInput.current?.click()}><ImagePlus /></button><input ref={imageInput} className="visually-hidden" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => void sendImage(event)} />
          <button className={`voice-message-trigger${recording ? ' recording' : ''}`} type="button" disabled={uploading || sending} aria-label={recording ? 'Остановить и отправить голосовое' : 'Записать голосовое'} onClick={recording ? finishRecording : () => void startRecording()}>{recording ? <Square size={17} fill="currentColor" /> : <Mic />}</button>
          ...
          <button type="submit" disabled={sending || uploading || recording || !draft.trim()} aria-label="Отправить"><Send /></button>
        </form>
      </div>
