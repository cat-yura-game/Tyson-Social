...
type ProfilePerson = { id: string; username: string; displayName: string; avatarKey: string | null; verified: number | boolean };
...
export function ProfilePage() {
  ...
  const [peopleDialog, setPeopleDialog] = useState<{ kind: 'followers' | 'following'; people: ProfilePerson[] | null; error: boolean } | null>(null);
  ...
  const openPeople = async (kind: 'followers' | 'following') => {
    setPeopleDialog({ kind, people: null, error: false });
    try {
      const result = await apiRequest<{ people: ProfilePerson[] }>(`/users/${encodeURIComponent(profile.username)}/${kind}`);
      setPeopleDialog((current) => current?.kind === kind ? { kind, people: result.people, error: false } : current);
    } catch {
      setPeopleDialog((current) => current?.kind === kind ? { kind, people: [], error: true } : current);
    }
  };
  return <section className="profile-page">
    ...
    <header className="profile-header">
      ...
      <div className="profile-stats"><span><strong>{posts.length}</strong> публикаций</span><button type="button" onClick={() => void openPeople('followers')}><strong>{profile.followerCount}</strong> подписчиков</button><button type="button" onClick={() => void openPeople('following')}><strong>{profile.followingCount}</strong> подписок</button></div>
    </header>
    ...
    {peopleDialog && createPortal(<div className="profile-people-backdrop" role="presentation" onClick={() => setPeopleDialog(null)}><section className="profile-people-modal" role="dialog" aria-modal="true" aria-label={peopleDialog.kind === 'followers' ? 'Подписчики' : 'Подписки'} onClick={(event) => event.stopPropagation()}><header><h2>{peopleDialog.kind === 'followers' ? 'Подписчики' : 'Подписки'}</h2><button type="button" aria-label="Закрыть" onClick={() => setPeopleDialog(null)}><X /></button></header><div className="profile-people-list">{peopleDialog.people === null ? <p>Загрузка…</p> : peopleDialog.error ? <p>Не удалось загрузить список.</p> : peopleDialog.people.length ? peopleDialog.people.map((person) => <Link key={person.id} to={`/profile/${encodeURIComponent(person.username)}`} onClick={() => setPeopleDialog(null)}><span className="avatar avatar-small">{person.avatarKey ? <img className="avatar-image" src={mediaUrl(person.avatarKey) ?? ''} alt="" /> : person.displayName.slice(0, 1).toUpperCase()}</span><span><strong>{person.displayName}{Boolean(person.verified) && <BadgeCheck className="verified" size={15} />}</strong><small>@{person.username}</small></span></Link>) : <p>{peopleDialog.kind === 'followers' ? 'Подписчиков пока нет.' : 'Пользователь пока ни на кого не подписан.'}</p>}</div></section></div>, document.body)}
