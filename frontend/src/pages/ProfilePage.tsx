import { AtSign, BadgeCheck, BarChart3, CalendarDays, Copy, Gift, MailWarning, MessageCircle, QrCode, Repeat2, Send, UserCheck, UserPlus, X } from 'lucide-react';
...
export function ProfilePage() {
  ...
  useEffect(() => { if (user) void apiRequest<{ linked: boolean }>('/auth/telegram/status').then((result) => setTelegramLinked(result.linked)).catch(() => undefined); }, [user?.id]);
  ...
  const resendEmailCode = async () => { setEmailActionPending(true); try { await apiRequest('/auth/email/resend', { method: 'POST' }); setEmailMessage('Код отправлен на вашу почту.'); } finally { setEmailActionPending(false); } };
  const changeEmail = async () => { const email = window.prompt('Введите правильный email', user?.email ?? '')?.trim(); if (!email) return; setEmailActionPending(true); try { await apiRequest('/auth/email/change', { method: 'POST', body: JSON.stringify({ email }) }); setEmailMessage('Почта изменена. Код отправлен на новый адрес.'); } finally { setEmailActionPending(false); } };
  const confirmTelegram = async () => { setEmailActionPending(true); try { const result = await apiRequest<{ authorizationUrl: string }>('/auth/telegram/start', { method: 'POST', body: JSON.stringify({ action: 'link' }) }); window.location.assign(result.authorizationUrl); } finally { setEmailActionPending(false); } };
  return <section className="profile-page">
    ...
    {isOwner && !user?.emailVerified && <section className="email-verification-card"><div><MailWarning size={22} /><div><strong>Нужно подтвердить почту</strong><p>{telegramLinked ? 'Telegram уже подтверждён, но email пока не подтверждён.' : 'Подтвердите email, чтобы защитить аккаунт.'}</p></div></div><div className="email-verification-actions"><button type="button" disabled={emailActionPending} onClick={() => void resendEmailCode()}>Выслать код</button><button type="button" disabled={emailActionPending} onClick={() => void changeEmail()}>Изменить почту</button><button type="button" disabled={emailActionPending || telegramLinked} onClick={() => void confirmTelegram()}><Send size={15} />{telegramLinked ? 'Telegram подтверждён' : 'Через Telegram'}</button></div>{emailMessage && <p>{emailMessage}</p>}</section>}
