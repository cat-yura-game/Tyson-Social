...
export function ProfilePage() {
  ...
  return <section className="profile-page">
    ...
    <header className="profile-header">
      ...
      <div className="profile-copy"><h1>{profile.displayName}{profile.verified && <BadgeCheck className="verified" size={21} aria-label="Подтверждённый аккаунт" />}{gifts.find((gift) => gift.worn) && <img className="profile-worn-gift" src={gifts.find((gift) => gift.worn)?.image} alt="Надетый подарок" />}</h1><div className="profile-identity"><p>@{profile.username}</p><small className="profile-last-seen">{lastSeen}</small></div><p className="profile-bio">{profile.bio || 'Пользователь пока ничего о себе не рассказал.'}</p>{birthday && <span><CalendarDays size={16} />{birthday}</span>}<span><CalendarDays size={16} />В Tyson с {joined}</span></div>
