...
export function ProfilePage() {
  ...
  return <section className="profile-page">
    ...
    {selectedAlias && <div className="alias-modal-backdrop" role="presentation" onClick={() => setSelectedAlias(null)}><section className="alias-modal" role="dialog" aria-modal="true" aria-label="Коллекционный username" onClick={(event) => event.stopPropagation()}><button className="alias-modal-close" type="button" aria-label="Закрыть" onClick={() => setSelectedAlias(null)}><X /></button><span className="alias-modal-icon"><AtSign size={38} /></span><h2>@{selectedAlias.username}</h2><strong>Коллекционный username</strong><p>Принадлежит пользователю <b>{profile.displayName}</b></p><p>Имя приобретено {new Intl.DateTimeFormat('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' }).format(new Date(selectedAlias.createdAt))} за <b>{selectedAlias.purchasePrice} 💎</b>.</p><button className="alias-copy-button" type="button" onClick={() => void navigator.clipboard?.writeText(`${window.location.origin}/profile/${selectedAlias.username}`)}><Copy size={17} />Копировать ссылку</button></section></div>}
  </section>;
}
