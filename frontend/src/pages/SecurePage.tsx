...
type SecureState = { active: boolean; expiresAt: string | null; whiteListActive: boolean; whiteListExpiresAt: string | null; whiteListCost: number; plans: Record<Plan, { cost: number; days: number; label: string }> };
...
export function SecurePage() {
  ...
  const buyWhiteList = async () => {
    setPending('link'); setError(null); setMessage(null);
    try { await apiRequest('/secure/white-list/purchase', { method: 'POST' }); await load(); window.dispatchEvent(new Event('diamonds-changed')); setMessage('Белые списки подключены до окончания Secure-подписки.'); }
    catch (caught) { setError(caught instanceof ApiError ? caught.message : 'Не удалось подключить белые списки.'); }
    finally { setPending(null); }
  };
  return <section className="surface-page narrow-page secure-page">
    ...
    <section className="secure-hero"><ShieldCheck size={30} /><div><span>Защищённое подключение</span><h2>Tyson Secure</h2><p>Обычные локации и автоподбор. Белые списки подключаются отдельно.</p></div></section>
    {error && <p className="form-error" role="alert">{error}</p>}{message && <p className="form-success">{message}</p>}
    {secure?.active ? <><section className="secure-active"><div><span>Подписка активна</span><h2>До {secure.expiresAt ? new Date(secure.expiresAt).toLocaleDateString('ru-RU') : ''}</h2><p>Автоподбор, Нидерланды и Германия доступны в одной ссылке.</p></div><button className="primary-button" type="button" disabled={pending === 'link'} onClick={() => void getLink()}>{pending === 'link' ? 'Готовим…' : <><Copy size={17} />Скопировать ссылку</>}</button>{accessUrl && <code>{accessUrl}</code>}</section><section className="secure-white-list"><div><p className="eyebrow">Дополнение</p><h2>Белые списки</h2><p>{secure.whiteListActive ? `Подключены до ${new Date(secure.whiteListExpiresAt ?? '').toLocaleDateString('ru-RU')}. В ссылке появятся профили России и Нидерландов.` : `Добавьте профили России и Нидерландов до окончания текущей подписки за ${secure.whiteListCost} 💎.`}</p></div>{secure.whiteListActive ? <span>Подключено</span> : <button type="button" disabled={pending !== null} onClick={() => void buyWhiteList()}>{pending === 'link' ? '…' : <>Подключить · <Gem size={15} />{secure.whiteListCost}</>}</button>}</section></> : <section className="secure-plans"><div><p className="eyebrow">Подписка</p><h2>Выберите срок</h2><p>Доступ к автоподбору, Нидерландам и Германии в INCY.</p></div>{secure && (Object.entries(secure.plans) as Array<[Plan, SecureState['plans'][Plan]]>).map(([key, plan]) => <button key={key} type="button" disabled={pending !== null} onClick={() => void purchase(key)}><span><strong>{plan.label}</strong><small>{plan.days === 1 ? 'Попробовать Secure' : 'Продление добавится к сроку'}</small></span><b>{pending === key ? '…' : <><Gem size={16} />{plan.cost}</>}</b></button>)}<small className="secure-notice"><Check size={14} />Белые списки можно подключить после покупки Secure за 20 💎. Если профиль не работает, обновите подписку и попробуйте другую локацию.</small></section>}
