...
type UserGift = { id: string; title: string; serialNumber: number; maxSupply: number; isCollectible: boolean; accentColor: string; worn: boolean; activeListingId: string | null; variant: string | null; image: string; upgradePrice: number; collectibleVariantNumber: number | null };
...
export function GiftsPage() {
  ...
  return <section className="gifts-page">
    ...
    <h2 className="gifts-title">Вторичный рынок</h2><div className="gift-collection">{listings.length ? listings.map((listing) => { const mine = owned.some((gift) => gift.id === listing.gift.id && gift.activeListingId === listing.id); return <article className={listing.gift.isCollectible ? 'owned-gift collectible' : 'owned-gift'} style={{ '--gift-accent': listing.gift.accentColor } as CSSProperties} key={listing.id}><img src={listing.gift.image} alt={listing.gift.title} /><div><h3>{listing.gift.title}</h3><p>Serial #{listing.gift.serialNumber} · @{listing.seller.username}</p><strong><Diamond size={15} />{listing.price}</strong>{mine ? <button type="button" disabled={pending !== null} onClick={() => void cancelListing(listing.id)}>Снять с продажи</button> : <button type="button" disabled={pending !== null} onClick={() => void buyListing(listing)}>{pending === listing.id ? 'Покупаем…' : 'Купить'}</button>}</div></article>; }) : <p className="empty-profile">На рынке пока нет подарков.</p>}</div>
