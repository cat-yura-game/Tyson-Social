...
export function GiftsPage() {
  ...
  return <section className="gifts-page">
    ...
    {selected && <GiftDetailsModal gift={selected.gift} owner={selected.owner} mine={selected.mine} onClose={() => setSelected(null)} onTransfer={() => { const gift = owned.find((item) => item.id === selected.gift.id); if (gift) void transfer(gift); }} onVisibility={() => { const gift = owned.find((item) => item.id === selected.gift.id); if (gift) void toggleVisibility(gift); }} onSell={() => { const gift = owned.find((item) => item.id === selected.gift.id); if (gift) void listGift(gift); }} />}
  </section>;
}
