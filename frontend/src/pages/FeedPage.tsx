...
export function FeedPage() {
  ...
  return <div className="feed-page">
    <StoriesBar />
    <header className="page-heading feed-heading"><div><p className="eyebrow">Персональная лента</p><h1>{view === 'fresh' ? 'Свежие публикации' : view === 'following' ? 'Публикации подписок' : 'Интересное для вас'}</h1></div><div className="feed-heading-actions"><button className="feed-search-button" type="button" onClick={() => setShowSearch(true)}><Search size={19} /><span>Поиск</span></button><button className="filter-button" type="button" onClick={() => setShowPreferences(true)}><SlidersHorizontal size={18} /><span>Настроить</span></button></div></header>
