...
export function FeedPage() {
  ...
  return <div className="feed-page">
    ...
    <section className="feed-list" aria-label="Лента публикаций">{loading ? <div className="feed-empty">Загрузка…</div> : posts.length ? posts.map((post) => <PostCard key={post.id} post={post} />) : <div className="feed-empty"><strong>{view === 'following' ? 'В подписках пока пусто' : 'Лента пока пуста'}</strong><span>{view === 'following' ? 'Подпишитесь на интересных авторов — их посты появятся здесь.' : 'Создайте первый пост в Tyson.'}</span></div>}</section>
