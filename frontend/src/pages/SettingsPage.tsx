...
export function SettingsPage() {
  ...
  return <section className="surface-page narrow-page settings-page settings-detail-page">
    ...
    <section className="email-settings" aria-labelledby="email-settings-title"><div className="email-settings-head"><span><Mail size={20} /></span><div><p className="eyebrow">Безопасность аккаунта</p><h2 id="email-settings-title">Электронная почта</h2><p>На неё приходят коды подтверждения и важные уведомления.</p></div></div><div className="email-settings-address"><span className="email-settings-logo"><img src="/logo.png" alt="" /></span><span><strong>{user.email}</strong><small>{user.emailVerified ? 'Почта подтверждена' : 'Нужно подтвердить почту'}</small></span>{user.emailVerified && <BadgeCheck aria-label="Почта подтверждена" size={20} />}</div>{!changingEmail ? <div className="email-settings-actions">{!user.emailVerified && <button className="secondary-button" type="button" disabled={emailVerificationPending} onClick={() => void sendEmailCode()}>Выслать код</button>}<button className="email-change-button" type="button" disabled={emailVerificationPending} onClick={() => { setNextEmail(user.email); setChangingEmail(true); setError(null); }}>Изменить почту</button></div> : <form className="change-email-form" onSubmit={(event) => void changeVerificationEmail(event)}><label><span>Новый адрес почты</span><input autoFocus type="email" autoComplete="email" value={nextEmail} onChange={(event) => setNextEmail(event.target.value)} placeholder="name@example.com" required /></label><p>Мы вышлем код на новый адрес. Он станет основным только после подтверждения.</p><div><button className="secondary-button" type="button" disabled={emailVerificationPending} onClick={() => setChangingEmail(false)}>Отмена</button><button className="email-change-button" type="submit" disabled={emailVerificationPending}>{emailVerificationPending ? 'Отправляем…' : 'Продолжить'}</button></div></form>}{!user.emailVerified && <div className="email-verification-actions"><button type="button" disabled={telegramPending || telegramStatus?.linked} onClick={() => void connectTelegram()}><Send size={15} />{telegramStatus?.linked ? 'Telegram подтверждён' : 'Подтвердить через Telegram'}</button></div>}{error && <p className="form-error" role="alert">{error}</p>}</section>
