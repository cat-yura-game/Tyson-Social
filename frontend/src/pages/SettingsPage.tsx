...
export function SettingsPage() {
  ...
  return <section className="surface-page narrow-page settings-page settings-detail-page">
    ...
    <section className="profile-color-settings" aria-labelledby="profile-color-title"><div><p className="eyebrow">Персонализация</p><h2 id="profile-color-title">Цвет шапки профиля</h2><p>Этот цвет увидят все, кто откроет ваш профиль.</p></div><div className="profile-color-options" role="radiogroup" aria-label="Цвет шапки профиля">{([['forest', 'Лесной'], ['ocean', 'Океан'], ['sunset', 'Закат'], ['violet', 'Фиолетовый'], ['rose', 'Розовый'], ['graphite', 'Графит']] as const).map(([value, label]) => <button key={value} type="button" role="radio" aria-checked={profileColor === value} className={profileColor === value ? `selected color-${value}` : `color-${value}`} onClick={() => setProfileColor(value)}><span /><small>{label}</small></button>)}</div></section>
