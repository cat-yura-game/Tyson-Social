import { ArrowLeft, BadgeCheck, BatteryMedium, BellRing, Camera, ChevronRight, CircleDashed, Database, Gem, LockKeyhole, MailWarning, Monitor, MonitorSmartphone, Moon, QrCode, RefreshCw, Save, Send, Settings, ShieldCheck, Sparkles, Sun, Trash2, Unlink, UserPlus, UserRound, Volume2 } from 'lucide-react';
...
export function SettingsPage() {
  ...
  const changeVerificationEmail = async () => { const email = window.prompt('Введите правильный email', user.email)?.trim(); if (!email) return; setEmailVerificationPending(true); try { await apiRequest('/auth/email/change', { method: 'POST', body: JSON.stringify({ email }) }); await refresh(); navigate('/verify-email'); } catch (caught) { setError(caught instanceof ApiError ? caught.message : 'Не удалось изменить почту.'); } finally { setEmailVerificationPending(false); } };
  ...
  return <section className="surface-page narrow-page settings-page settings-detail-page">
    ...
    {!user.emailVerified && <section className="email-verification-card"><div><MailWarning size={22} /><div><strong>Нужно подтвердить почту</strong><p>{telegramStatus?.linked ? 'Telegram подключён, но email пока не подтверждён.' : 'Подтвердите email, чтобы защитить аккаунт.'}</p></div></div><div className="email-verification-actions"><button type="button" disabled={emailVerificationPending} onClick={() => void sendEmailCode()}>Выслать код</button><button type="button" disabled={emailVerificationPending} onClick={() => void changeVerificationEmail()}>Изменить почту</button><button type="button" disabled={telegramPending || telegramStatus?.linked} onClick={() => void connectTelegram()}><Send size={15} />{telegramStatus?.linked ? 'Telegram подтверждён' : 'Через Telegram'}</button></div>{error && <p className="form-error" role="alert">{error}</p>}</section>}
