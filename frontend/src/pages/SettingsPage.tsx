import { ArrowDown, ArrowLeft, ArrowUp, BadgeCheck, BatteryMedium, BellRing, Camera, ChevronRight, CircleDashed, Database, Gem, LifeBuoy, LockKeyhole, Mail, Monitor, MonitorSmartphone, Moon, QrCode, RefreshCw, Save, Send, Settings, ShieldCheck, Sparkles, Sun, Trash2, Unlink, UserPlus, UserRound, Volume2 } from 'lucide-react';
...
export function SettingsPage() {
  ...
  const [uiScale, setUiScaleValue] = useState(() => getUiScale());
  ...
  const [loginApproval, setLoginApproval] = useState<{ enabled: boolean; method: 'telegram' | 'email' | 'both' }>({ enabled: false, method: 'email' });
  const [loginApprovalPending, setLoginApprovalPending] = useState(false);
  ...
  const [aliases, setAliases] = useState<Array<{ id: string; username: string; sortOrder: number }>>([]);
  ...
  useEffect(() => { if (user) void apiRequest<typeof loginApproval>('/users/me/login-approval-settings').then(setLoginApproval).catch(() => undefined); }, [user]);
  const saveLoginApproval = async (next: typeof loginApproval) => { setLoginApprovalPending(true); try { const saved = await apiRequest<typeof loginApproval>('/users/me/login-approval-settings', { method: 'PUT', body: JSON.stringify(next) }); setLoginApproval(saved); } catch (caught) { setError(caught instanceof ApiError ? caught.message : 'Не удалось сохранить подтверждение входа.'); } finally { setLoginApprovalPending(false); } };
  ...
  const loadAliases = async () => { const result = await apiRequest<{ aliases: Array<{ id: string; username: string; sortOrder: number }> }>('/users/me/aliases'); setAliases(result.aliases); };
  ...
  const saveProfileColor = async (color: string) => {
    ...
    setProfileColor(color); setProfileColorPending(true); setError(null);
    try { await apiRequest('/users/me', { method: 'PATCH', body: JSON.stringify({ profileColor: color }) }); await refresh(); setMessage('Цвет шапки профиля изменён.'); }
    catch (caught) { setProfileColor(user.profileColor ?? 'forest'); setError(caught instanceof ApiError ? caught.message : 'Не удалось изменить цвет шапки.'); }
    finally { setProfileColorPending(false); }
  };
  ...
  const chooseUiScale = (value: number) => {
    setUiScaleValue(value);
    setUiScale(value);
  };

  const chooseMobileLastTab = (value: MobileLastTab) => {
    setMobileLastTabValue(value);
    setMobileLastTab(value);
  };
  ...
  const moveAlias = async (index: number, direction: -1 | 1) => {
    const target = index + direction; if (target < 0 || target >= aliases.length || aliasPending) return;
    const previous = aliases; const next = [...aliases]; [next[index], next[target]] = [next[target]!, next[index]!];
    setAliases(next); setAliasPending(true); setError(null);
    try { await apiRequest('/users/me/aliases/order', { method: 'PUT', body: JSON.stringify({ aliasIds: next.map((alias) => alias.id) }) }); }
    catch (caught) { setAliases(previous); setError(caught instanceof ApiError ? caught.message : 'Не удалось изменить порядок username.'); }
    finally { setAliasPending(false); }
  };
  ...
  const sendEmailCode = async () => { setEmailVerificationPending(true); try { await apiRequest('/auth/email/resend', { method: 'POST' }); navigate('/verify-email'); } catch (caught) { setError(caught instanceof ApiError ? caught.message : 'Не удалось отправить код.'); } finally { setEmailVerificationPending(false); } };
  const changeVerificationEmail = async (event: FormEvent) => { event.preventDefault(); const email = nextEmail.trim(); if (!email) return; setEmailVerificationPending(true); setError(null); try { await apiRequest('/auth/email/change', { method: 'POST', body: JSON.stringify({ email }) }); navigate('/verify-email?change=1'); } catch (caught) { setError(caught instanceof ApiError ? caught.message : 'Не удалось изменить почту.'); } finally { setEmailVerificationPending(false); } };
  ...
  return <section className="surface-page narrow-page settings-page settings-detail-page">
    ...
    {settingsCategory === 'appearance' && <><section className="theme-settings" aria-labelledby="theme-settings-title"><div><p className="eyebrow">Оформление</p><h2 id="theme-settings-title">Тема Tyson</h2></div><div className="theme-options" role="radiogroup" aria-label="Тема интерфейса"><button type="button" role="radio" aria-checked={theme === 'system'} className={theme === 'system' ? 'selected' : ''} onClick={() => chooseTheme('system')}><Monitor size={18} />Как в системе</button><button type="button" role="radio" aria-checked={theme === 'light'} className={theme === 'light' ? 'selected' : ''} onClick={() => chooseTheme('light')}><Sun size={18} />Светлая</button><button type="button" role="radio" aria-checked={theme === 'dark'} className={theme === 'dark' ? 'selected' : ''} onClick={() => chooseTheme('dark')}><Moon size={18} />Тёмная</button></div></section><section className="interface-scale-settings" aria-labelledby="interface-scale-title"><div><p className="eyebrow">Удобство</p><h2 id="interface-scale-title">Размер интерфейса <strong>{uiScale}%</strong></h2><p>Увеличьте Tyson, если на большом телефоне элементы кажутся слишком маленькими.</p></div><div className="interface-scale-control"><span>Меньше</span><input type="range" min="90" max="125" step="5" value={uiScale} aria-label="Размер интерфейса" onChange={(event) => chooseUiScale(Number(event.target.value))} /><span>Больше</span></div></section><section className="mobile-last-tab-settings" aria-labelledby="mobile-last-tab-title"><div><p className="eyebrow">Мобильная навигация</p><h2 id="mobile-last-tab-title">Последняя вкладка</h2><p>Выберите, что открывает последняя кнопка нижней панели.</p></div><div className="theme-options" role="radiogroup" aria-label="Последняя вкладка"><button type="button" role="radio" aria-checked={mobileLastTab === 'settings'} className={mobileLastTab === 'settings' ? 'selected' : ''} onClick={() => chooseMobileLastTab('settings')}><Settings size={18} />Настройки</button><button type="button" role="radio" aria-checked={mobileLastTab === 'profile'} className={mobileLastTab === 'profile' ? 'selected' : ''} onClick={() => chooseMobileLastTab('profile')}><UserRound size={18} />Профиль</button></div></section></>}
    {settingsCategory === 'profile' && <>
    <section className="login-approval-settings" aria-labelledby="login-approval-title"><div><p className="eyebrow">Безопасность аккаунта</p><h2 id="login-approval-title"><ShieldCheck size={19} />Подтверждение входа</h2><p>Функция выключена по умолчанию. При новом входе Tyson запросит подтверждение через выбранный канал.</p></div><label className="settings-switch"><input type="checkbox" checked={loginApproval.enabled} disabled={loginApprovalPending} onChange={(event) => void saveLoginApproval({ ...loginApproval, enabled: event.target.checked })} /><span aria-hidden="true" /><b>{loginApproval.enabled ? 'Включено' : 'Выключено'}</b></label><label><span>Канал подтверждения</span><select value={loginApproval.method} disabled={loginApprovalPending || !loginApproval.enabled} onChange={(event) => void saveLoginApproval({ ...loginApproval, method: event.target.value as typeof loginApproval.method })}><option value="telegram">Telegram — кнопка «Разрешить»</option><option value="email">Email — одноразовый код</option><option value="both">Telegram и email</option></select></label></section>
    <section className="email-settings" aria-labelledby="email-settings-title"><div className="email-settings-head"><span><Mail size={20} /></span><div><p className="eyebrow">Безопасность аккаунта</p><h2 id="email-settings-title">Электронная почта</h2><p>На неё приходят коды подтверждения и важные уведомления.</p></div></div><div className="email-settings-address"><span><strong>{user.email}</strong><small>{user.emailVerified ? 'Почта подтверждена' : 'Нужно подтвердить почту'}</small></span>{user.emailVerified && <BadgeCheck aria-label="Почта подтверждена" size={20} />}</div>{!changingEmail ? <div className="email-settings-actions">{!user.emailVerified && <button className="secondary-button" type="button" disabled={emailVerificationPending} onClick={() => void sendEmailCode()}>Выслать код</button>}<button className="email-change-button" type="button" disabled={emailVerificationPending} onClick={() => { setNextEmail(user.email); setChangingEmail(true); setError(null); }}>Изменить почту</button></div> : <form className="change-email-form" onSubmit={(event) => void changeVerificationEmail(event)}><label><span>Новый адрес почты</span><input autoFocus type="email" autoComplete="email" value={nextEmail} onChange={(event) => setNextEmail(event.target.value)} placeholder="name@example.com" required /></label><p>Мы вышлем код на новый адрес. Он станет основным только после подтверждения.</p><div><button className="secondary-button" type="button" disabled={emailVerificationPending} onClick={() => setChangingEmail(false)}>Отмена</button><button className="email-change-button" type="submit" disabled={emailVerificationPending}>{emailVerificationPending ? 'Отправляем…' : 'Продолжить'}</button></div></form>}{!user.emailVerified && <div className="email-verification-actions"><button type="button" disabled={telegramPending || telegramStatus?.linked} onClick={() => void connectTelegram()}><Send size={15} />{telegramStatus?.linked ? 'Telegram подтверждён' : 'Подтвердить через Telegram'}</button></div>}{error && <p className="form-error" role="alert">{error}</p>}</section>
    <section className="profile-color-settings" aria-labelledby="profile-color-title"><div><p className="eyebrow">Персонализация</p><h2 id="profile-color-title">Цвет шапки профиля</h2><p>Этот цвет увидят все, кто откроет ваш профиль.</p></div><div className="profile-color-options" role="radiogroup" aria-label="Цвет шапки профиля">{([['forest', 'Лесной'], ['ocean', 'Океан'], ['sunset', 'Закат'], ['violet', 'Фиолетовый'], ['rose', 'Розовый'], ['graphite', 'Графит']] as const).map(([value, label]) => <button key={value} type="button" disabled={profileColorPending} role="radio" aria-checked={profileColor === value} className={profileColor === value ? `selected color-${value}` : `color-${value}`} onClick={() => void saveProfileColor(value)}><span /><small>{label}</small></button>)}</div></section>
    ...
    <form className="settings-form" onSubmit={(event) => void save(event)}>
      ...
      <label><span>Имя пользователя</span><input required minLength={3} maxLength={30} pattern="[A-Za-z](?:[A-Za-z0-9]|_[A-Za-z0-9])*" title="Начните с буквы. Нельзя использовать несколько подчёркиваний подряд или подчёркивание в конце." disabled={!user.usernameChangeAvailable} value={username} onChange={(event) => setUsername(event.target.value)} /><small>{user.usernameChangeAvailable ? 'Начинается с буквы; без повторяющихся подчёркиваний и подчёркивания в конце. Изменить можно только один раз.' : 'Вы уже использовали единственную смену username.'}</small></label>
    ...
    <section className="alias-settings"><div><p className="eyebrow">Имена пользователя</p><h2>Дополнительные username</h2><p>Основной username остаётся прежним. Каждый дополнительный стоит 50 💎.</p></div><form onSubmit={(event) => void addAlias(event)}><span>@</span><input minLength={3} maxLength={30} pattern="[A-Za-z](?:[A-Za-z0-9]|_[A-Za-z0-9])*" title="Начните с буквы. Нельзя использовать несколько подчёркиваний подряд или подчёркивание в конце." value={newAlias} onChange={(event) => setNewAlias(event.target.value.replace(/^@/u, ''))} placeholder="новое_имя" /><button type="submit" disabled={aliasPending}>Добавить · 50 💎</button></form>{aliases.length > 0 && <div className="alias-list">{aliases.map((alias, index) => <div key={alias.id}><span>@{alias.username}</span><div className="alias-order-actions"><button type="button" disabled={aliasPending || index === 0} aria-label={`Поднять @${alias.username}`} title="Поднять" onClick={() => void moveAlias(index, -1)}><ArrowUp size={15} /></button><button type="button" disabled={aliasPending || index === aliases.length - 1} aria-label={`Опустить @${alias.username}`} title="Опустить" onClick={() => void moveAlias(index, 1)}><ArrowDown size={15} /></button><button className="alias-delete-button" type="button" disabled={aliasPending} onClick={() => void deleteAlias(alias.id)}><Trash2 size={15} />Удалить</button></div></div>)}</div>}<small>Стрелками выберите порядок отображения. Можно добавить до 20 дополнительных username.</small></section>
    ...
    {verifiedAccounts && (verifiedAccounts.canCreate || verifiedAccounts.accounts.length > 0) && <section className="linked-accounts-settings" aria-labelledby="linked-accounts-title">
      ...
      {verifiedAccounts.canCreate && <form className="linked-account-form" onSubmit={(event) => void createLinkedAccount(event)}>
        ...
        <input required minLength={3} maxLength={30} pattern="[A-Za-z](?:[A-Za-z0-9]|_[A-Za-z0-9])*" title="Начните с буквы. Нельзя использовать несколько подчёркиваний подряд или подчёркивание в конце." placeholder="Username" value={linkedAccount.username} onChange={(event) => setLinkedAccount({ ...linkedAccount, username: event.target.value })} />
