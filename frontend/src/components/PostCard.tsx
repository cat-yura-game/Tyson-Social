...
export function PostCard({ post, onDeleted }: { post: Post; onDeleted?: (postId: string) => void }) {
  ...
  const [reportResult, setReportResult] = useState<{ status: 'removed' | 'review' | 'no_violation'; message: string } | null>(null);
  ...
  const submitReport = async (event: React.FormEvent) => {
    ...
    try {
      const result = await apiRequest<{ status: 'removed' | 'review' | 'no_violation'; message: string }>(`/posts/${encodeURIComponent(post.id)}/report`, {
