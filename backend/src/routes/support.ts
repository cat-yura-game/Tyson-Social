...
async function notifyOwner(env: Env, db: D1Database, ticketId: string, category: string, question: string, screenshotKey?: string, aiAnswer?: string | null) {
  ...
  await fetch(`https://api.telegram.org/bot${env.TELEGRAM_BOT_TOKEN}/sendMessage`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ chat_id: owner.chatId, text: `🛟 Обращение в поддержку Tyson\n\nКатегория: ${category}\nID: ${ticketId}\n\nСообщение пользователя:\n${question}${aiAnswer ? `\n\nОтвет Tyson AI:\n${aiAnswer}` : ''}${screenshotKey ? '\n\n📎 Прикреплён скриншот.' : ''}\n\nОтветьте командой:\n/support_reply ${ticketId} ваш ответ` }) });
}
...
supportRoutes.post('/tickets/:id/escalate', async (c) => { const user = userOf(c); if (!user) return fail(c, 401, 'AUTH_REQUIRED', 'Authentication is required.'); const ticket = await c.env.DB.prepare('SELECT id, category, question, ai_answer AS aiAnswer, screenshot_key AS screenshotKey FROM support_tickets WHERE id = ? AND user_id = ?').bind(c.req.param('id'), user.id).first<{id:string;category:string;question:string;aiAnswer?:string|null;screenshotKey?:string}>(); if (!ticket) return fail(c, 404, 'TICKET_NOT_FOUND', 'Обращение не найдено.'); await c.env.DB.prepare("UPDATE support_tickets SET status = 'open', updated_at = ? WHERE id = ?").bind(new Date().toISOString(), ticket.id).run(); await notifyOwner(c.env, c.env.DB, ticket.id, ticket.category, ticket.question ?? '', ticket.screenshotKey, ticket.aiAnswer); return ok(c, { escalated: true }); });
