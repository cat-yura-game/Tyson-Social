...
function reportedPostNotificationText(reason: string, content?: { title?: string; body?: string }): string {
  const title = content?.title?.trim();
  const firstLine = content?.body?.split(/\r?\n/u).map((line) => line.trim()).find(Boolean);
  const subject = (title || firstLine || '').slice(0, 180);
  return [
    '🛡️ Публикация удалена после жалобы',
    '',
    'Неизвестный пользователь пожаловался на вашу публикацию. Личность отправителя жалобы не раскрывается.',
    '',
    '✓ Проверено нейросетью Tyson',
    'Это автоматическое решение системы безопасности, а не сообщение от человека.',
    '',
    'Причина удаления:',
    reason.trim() || 'Публикация нарушает правила безопасности Tyson.',
    ...(subject ? ['', 'Публикация:', `«${subject}${subject.length === 180 ? '…' : ''}»`] : []),
    '',
    'Если вы считаете, что произошла ошибка, напишите в поддержку для повторной проверки.',
    '',
    '— Защитник Tyson',
  ].join('\n');
}

async function sendSafetyMessage(env: Env, recipientUserId: string, text: string, pushBody: string): Promise<void> {
  ...
  const encrypted = await encryptCloudMessage(env, { type: 'support_notice', text });
  const statements = [];
  if (!existing) {
    statements.push(
  ...
  await sendPushToUser(env, recipientUserId, { title: safety.displayName, body: pushBody, url: `/messages?conversation=${encodeURIComponent(conversationId)}`, tag: `moderation-${conversationId}` });
}
...
export async function sendModerationMessage(
  ...
  try {
    await sendSafetyMessage(env, recipientUserId, notificationText(kind, reason, content), 'Публикация не прошла проверку');
  } catch (error) {
...
export async function sendReportedPostRemovalMessage(
  env: Env,
  recipientUserId: string,
  reason: string,
  content?: { title?: string; body?: string },
  ...
  try {
    await sendSafetyMessage(env, recipientUserId, reportedPostNotificationText(reason, content), 'Публикация удалена после проверки жалобы');
  } catch (error) {
    console.error(JSON.stringify({ event: 'reported_post_message_failed', recipientUserId, error: error instanceof Error ? error.message : 'unknown' }));
  }
}
